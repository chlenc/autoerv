...
// })
    ...
    bot.on('callback_query', query => {
        ...
        try {
            ...
            if (data.type == 'get_order') {
                ...
                if (data.per === 'driv')
                    var reply = keyboards.getDriverEndKey(data.to, query.from.id, data.mark);
                else
                    var reply = keyboards.getPassengerEndKey(query.from.id, data.to, data.mark);

                bot.editMessageReplyMarkup(reply.reply_markup, {
                    chat_id: chat.id,
                    message_id: message_id
                })
            }
    ...
    console.log('nearly collapsed '+e.toString())
}
